...
construtorTela(gameState, cenarioFalas, estadoAtual, i)

function construtorTela(gameState, cenarioFalas, estadoAtual, i){
...
gameState["fundo"].style.backgroundImage = (imagemFundo)
...
construtorDialogo(gameState,cenarioFalas, estadoAtual, i)
mudaFundo(gameState, cenarioFalas, estadoAtual, i)
console.log("CONSTRUIDO O FUNDO")
}

function construtorDialogo(gameState, cenarioFalas, estadoAtual, i){
const caixaDeDialogo = document.querySelector("#caixaDeDialogo")
gameState["fundo"].appendChild(caixaDeDialogo)
...
gameState["caixaDeDialogo"] = caixaDeDialogo
const dialogo = document.querySelector("#dialogo")
gameState["caixaDeDialogo"].appendChild(dialogo)
gameState["dialogo"] = dialogo
const texto = document.querySelector("#texto")
gameState["dialogo"].appendChild(texto)
gameState["texto"] = texto

const botaoNext = document.createElement("button")
botaoNext.setAttribute("id", "botaoNext")
botaoNext.innerHTML = "NEXT"
gameState["caixaDeDialogo"].appendChild(botaoNext)

atualizaTexto(gameState, cenarioFalas, estadoAtual, i)


console.log("CONSTRUIDO DIALOGO")
}

function mudaFundo(gameState, cenarioFalas, estadoAtual, i){
    gameState["imagemFundo"].src = cenarioFalas["cenario"][estadoAtual]["imagem"]
    gameState["fundo"].style.backgroundImage = imagemFundo
    ...
    i=0
    desaparece(gameState, cenarioFalas, estadoAtual, i)
    console.log("MUDA FUNDO FUNCIONA")
}

function atualizaTexto(gameState, cenarioFalas, estadoAtual, i){
    gameState["texto"].innerHTML = `<b><i>${cenarioFalas['cenario'][estadoAtual]['falas'][i]['autor']} - </i></b>${cenarioFalas['cenario'][estadoAtual]['falas'][i]['frase']}`
    console.log("ATUALIZA TEXTO FUNCIONA")
}

function desaparece(gameState, cenarioFalas, estadoAtual, i){
    if(cenarioFalas['cenario'][estadoAtual]['numeroDialogos'] >= i){
        gameState["caixaDeDialogo"].style.opacity = 1
        gameState["botaoOpcao1"].style.opacity = 0
        gameState["botaoOpcao2"].style.opacity = 0
    }
    if(cenarioFalas['cenario'][estadoAtual]['numeroDialogos'] < i){
        gameState["caixaDeDialogo"].style.opacity = 0
